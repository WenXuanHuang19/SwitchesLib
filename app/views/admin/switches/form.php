<?php
/**
 * Admin add/edit switch form. $switch is null on create, the row on edit.
 *
 * @var ?array $switch    the switch being edited, or null when creating
 * @var array  $designers designer rows for the dropdown
 * @var array  $errors    field-level validation errors
 * @var array  $old       previously submitted / existing values to repopulate
 */
$isEdit = $switch !== null;
$action = $isEdit ? url('/admin/switches/' . $switch['id']) : url('/admin/switches');
...
<h1><?= $isEdit ? 'Edit Switch' : 'Add Switch' ?></h1>
...
<form class="submit-form" method="post" action="<?= $action ?>" enctype="multipart/form-data">
    ...
    <fieldset>
        ...
        <label>Status
            <select name="status">
                <?php foreach (['approved', 'draft'] as $st): ?>
                    <option value="<?= e($st) ?>" <?= ($old['status'] ?? 'approved') === $st ? 'selected' : '' ?>>
                        <?= e(ucfirst($st)) ?>
                    </option>
                <?php endforeach; ?>
            </select>
    ...
    <fieldset>
        ...
        <?php if ($isEdit && !empty($switch['image_url'])): ?>
            <p class="current-image">
                Current image:
                <img src="<?= url($switch['image_url']) ?>" alt="<?= e($switch['name']) ?>" height="60">
            </p>
        <?php endif; ?>
        <label>Image (JPG, PNG or WebP)
    ...
    <button type="submit" class="btn"><?= $isEdit ? 'Save changes' : 'Create switch' ?></button>
</form>
